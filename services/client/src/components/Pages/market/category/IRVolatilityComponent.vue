...
// IR Volatility specific filters
const filters = {
  Name: {
    values: [
      'EUR.ESTR.VOLMATRIX',
...
<template>
  ...
  <div class="card">
    <div v-if="loading" class="flex flex-column align-items-center justify-content-center p-4 text-center text-color-secondary">
      ...
      <p class="mt-3 mb-3">Loading IR volatility data...</p>
    </div>
    ...
    <SimpleTable
      v-else
      :data="tableData"
      downloadFileName="ir_volatility_data"
    />
  </div>

  <!-- Future: IR Volatility Surface Visualization -->
  <!-- This section can be expanded to include IR vol surface plots -->
  <!--
  <div v-if="tableData.length > 0 && selectedFilters.Name" class="card">
    <div class="plot-header">
      <h5>IR Volatility Surface</h5>
    </div>
    <div class="ir-vol-surface-container">
      // Future: Add IR volatility surface plot here
    </div>
  </div>
  -->
</template>
