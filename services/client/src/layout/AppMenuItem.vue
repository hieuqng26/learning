...
watch(
  ...
  (newVal) => {
    ...
    // Update the active menu item in localStorage
    localStorage.setItem('activeMenuItem', newVal)
  }
)
const itemClick = (event, item) => {
  ...
  localStorage.setItem('activeMenuItem', foundItemKey)
}

const checkActiveRoute = (item) => {
  if (typeof item.to === 'string') {
    // Check the route by path
    return route.path === item.to
  } else if (item.to && item.to.name) {
    // Check the route by name
