...
LOGIT_POST_DELIVERY_INTERCEPT = 6.403
LOGIT_POST_DELIVERY_SLOPE = 0.032
LOGIT_PRE_POST_DELIVERY_INTERCEPT = 5.233
LOGIT_PRE_POST_DELIVERY_SLOPE = 0.010

# Response severity mapping
RESPONSE_SEVERITY = {"A": 1, "B": 2, "C": 3, "D": 4, "E": 5}
ORDINAL_TO_QUAL = {1: "A", 2: "B", 3: "C", 4: "D", 5: "E"}
QUAL_TO_ORDINAL = {"A": 1, "B": 2, "C": 3, "D": 4, "E": 5}

# Questions where response C should be changed to D
C_TO_D_QUESTIONS = [11, 15, 16, 17]

# MEV concepts where YoY is computed as absolute difference / 100
ABSOLUTE_YOY_CONCEPTS = [
    "Unemployment rate",
...
# Subset used in apply_transformations_single (excludes Subinvestment Grade EM Bond Index)
ABSOLUTE_YOY_CONCEPTS_SINGLE = [
    "Unemployment rate",
    "Investment Grade Emerging Markets Bond Index",
    "Eurozone Bond Index",
    "Monetary policy or key interest rate",
...
def pd_lookup_cg(pd_series, lk):
    """
    Map a series of PD values to (cg_num, cg_look_up) using the lookup table lk.
    Returns two lists: nums, cgs.
    """
    nums, cgs = [], []
    for val in pd_series:
        row = lk[(val >= lk["lower_bound"]) & (val <= lk["upper_bound"])]
        if not row.empty:
            nums.append(row.iloc[0]["cg_num"])
            cgs.append(row.iloc[0]["cg_look_up"])
        else:
            nums.append(None)
            cgs.append(None)
    return nums, cgs
...
def categorise_notch_diff(series):
    """
    Map a numeric notch-difference series to category labels.
    Used for both Qual and standalone CG impact columns.
    """
    return np.where(
        series >= 3.5,
        ">= 3.5 notches",
        np.where(
            ...
            np.where(
                series > 0,
                "<=1.5 notches",
                np.where(series < 0, "Upgrade", "Flat"),
            ),
        ),
    )


def impact_indicator(series):
    """
    Map a numeric notch-difference series to Downgrade / Upgrade / Flat.
    """
    return np.where(
        series > 0,
        "Downgrade",
...
def apply_transformations(
    ...
    if yoy is not None and not pd.isna(yoy):
        k = int(yoy)
        if concept in ABSOLUTE_YOY_CONCEPTS:
...
def _long_for(abc, concept, country):
...
def apply_transformations_single(
    abc, concept, country, tp, ma_quarters=None, yoy=None, lag=None
):
    df = _long_for(abc, concept, country)
    if df is None:
        return np.nan
    out = df["value"]
    ...
    if yoy is not None and pd.notna(yoy):
        if concept in ABSOLUTE_YOY_CONCEPTS_SINGLE:
...
def apply_transformations_weighted(
    abc, concept, countries, weights, tp, ma=None, yoy=None, lag=None
):
    available, wts = [], []
    for c in countries:
...
def load_and_transform_mev(scenario_file, beta_file_path, timepoint, avg_countries, raw_weights):
    """
    Load the scenario CSV and beta Excel (Sheet1), compute MEV transformations,
    compute GDP-weighted averages for Ave_ MEVs, and return the merged transformed_MEV df.
    """
    abc = pd.read_csv(scenario_file)
    MEV_LIST = abc["concept"].unique()

    # --- parse beta file ---
    beta = pd.read_excel(beta_file_path, sheet_name="Sheet1")
    beta = beta.copy()
    ...
    # --- compute GDP-weighted average values ---
    beta_for_ave = beta.copy()
    ...
    beta_for_ave["transformation_list"] = (
    ...
    return transformed_MEV


def load_and_prepare_scores(qual_factor_input_dir, project_dir, risk_file_dir, misc_input_dir, static_input_dir, beta_file):
    """
    Load score data, threshold data, risk data, model weights, and pd data.
    Returns out_with_thr_weights DataFrame ready for stress computation.
    Also returns the thr DataFrame for later re-use.
    """
    # --- load & standardize threshold data ---
    thr = clean_column_names(pd.read_excel(qual_factor_input_dir / beta_file, sheet_name="Sheet2"))
    ...
    risk_mi = clean_column_names(pd.read_csv(INPUT_DIR / "SF_DEC_2024_List.csv"))

    # change response C to D for question number 11
    score["non_fin_res_id"] = score["non_fin_res_id"].mask(
        (score["non_fin_ques_no"].isin(C_TO_D_QUESTIONS))
    ...
    out_post_exclusion["sev"] = out_post_exclusion["r_join"].map(RESPONSE_SEVERITY)
    ...
    # Read pd_data once with all needed columns
    pd_data = pd.read_csv(misc_input_dir / "sf_pd_data.csv")

    # Keep only exact columns for the first merge
    loan_flag_data = pd_data[["scorecard_seq_id", "loan_purpose"]].copy()
    ...
    return out_with_thr_weights, thr, pd_data


def compute_stressed_responses(pre_stress_df, transformed_mev_df, thr_df, timepoint, apply_score_distribution):
    """
    Run the ordinal regression stress computation.
    Returns the stressed responses DataFrame (previously written/read as stressed_responses.csv).
    """
    out_with_thr_weight = pre_stress_df.copy()
    ...
    df["stressed_qual_response"] = df["stressed_ordinal_response"].map(ORDINAL_TO_QUAL)

    # for missing 'ordinal_response', re-map from "r-join"
    df["ordinal_response"] = np.where(
        pd.isna(df["ordinal_response"]),
        df["r_join"].map(QUAL_TO_ORDINAL),
        df["ordinal_response"],
    )
...
def compute_qual_cg(stressed_df, misc_input_dir, static_input_dir, pd_data, output_dir, scenario_name, timepoint, output_individual_file):
    """
    Compute qual CG from stressed responses.
    Returns final qual CG DataFrame and the three KeyTable DataFrames.
    """
    df = stressed_df.copy()
    ...
    # Use the already-loaded pd_data (with all needed columns)
    pd_keep_full = pd_data[
    ...
    pd_keep_full["scorecard_seq_id"] = pd_keep_full["scorecard_seq_id"].astype(str)

    # Merge — always take PD file's values (drop old columns first if present)
    ...
    stressed_with_weights = stressed_with_weights.merge(
        pd_keep_full, on="scorecard_seq_id", how="left"
    )
    ...
    def calculate_unstressed_qual_pd(row):
        if row["loan_purpose"] == "Post-Delivery Only":
            a, b = LOGIT_POST_DELIVERY_INTERCEPT, LOGIT_POST_DELIVERY_SLOPE
        elif row["loan_purpose"] == "Pre+Post-Delivery":
            a, b = LOGIT_PRE_POST_DELIVERY_INTERCEPT, LOGIT_PRE_POST_DELIVERY_SLOPE
        else:
    ...
    un_qual_num, un_qual_cg = pd_lookup_cg(final["unstressed_qual_pd"], lk)
    final["unstressed_qual_cg_num"] = un_qual_num
    final["unstressed_qual_cg"] = un_qual_cg

    qual_num, qual_cg = pd_lookup_cg(final["qual_pd"], lk)
    ...
    final["Qual Impact_Indicator"] = impact_indicator(final["Qual Difference in Notches"])
    final["Category_Qual"] = categorise_notch_diff(final["Qual Difference in Notches"])
    ...
    df_KeyTable_QualCG = output_summary_table(
        ...
        "Category_Qual",
        outputIndividualFile=output_individual_file,
    )
    ...
    return final, df_KeyTable_QualCG, df_KeyTable_QualCGpostDelOnly, df_KeyTable_QualCGprePostDeliv


def compute_standalone_and_overrides(qual_cg_df, misc_input_dir, static_input_dir, new_input_dir, charterer_cg_input_dir, scenario_name, timepoint, output_individual_file, output_dir):
    """
    Compute standalone CG, apply AWS, parental support, sovereign ceiling overrides.
    Returns final_crg_post_overrides DataFrame and df_KeyTable_StandaloneCG, df_KeyTable_final_crg_post_overrides.
    """
    qual = qual_cg_df.copy()
    ...
    # --- Step 3: Model PD → CG ---
    unstressed_final_num, unstressed_final_cg = pd_lookup_cg(df["unstressed_model_pd"], lk)
    df["unstressed_final_cg_num"] = unstressed_final_num
    df["unstressed_final_cg"] = unstressed_final_cg

    final_num, final_cg = pd_lookup_cg(df["model_pd"], lk)
    df["final_cg_num"] = final_num
    df["final_cg"] = final_cg

    df["Difference in Notches"] = (df["final_cg_num"] - df["unstressed_final_cg_num"]) / 2
    df["Impact_Indicator"] = impact_indicator(df["Difference in Notches"])
    df["Category"] = categorise_notch_diff(df["Difference in Notches"])
    ...
    return final_crg_post_overrides, df_KeyTable_StandaloneCG, df_KeyTable_final_crg_post_overrides
...
def main():
    # --- Read scenario and compute raw GDP weights ---
    ...
    # Stage 1: MEV loading and transformation
    transformed_MEV = load_and_transform_mev(
        scenario_file,
        QUAL_FACTOR_INPUT_DIR / beta_file,
        TIMEPOINT,
        AVG_COUNTRIES,
        raw_weights,
    )

    # Stage 2: Load and prepare score data
    out_with_thr_weights, thr, pd_data = load_and_prepare_scores(
        QUAL_FACTOR_INPUT_DIR,
        PROJECT_DIR,
        RISK_FILE_DIR,
        MISC_INPUT_DIR,
        STATIC_INPUT_DIR,
    ...
    # Stage 3: Compute stressed responses (ordinal regression)
    stressed_df = compute_stressed_responses(
        out_with_thr_weights,
        transformed_MEV,
        thr,
        TIMEPOINT,
        apply_score_distribution_not_max_prob_score,
    )

    # Stage 4: Compute qual CG
    qual_cg_df, df_KeyTable_QualCG, df_KeyTable_QualCGpostDelOnly, df_KeyTable_QualCGprePostDeliv = compute_qual_cg(
        stressed_df,
        MISC_INPUT_DIR,
        STATIC_INPUT_DIR,
        pd_data,
    ...
    # Stage 5: Standalone CG + overrides
    final_crg_post_overrides, df_KeyTable_StandaloneCG, df_KeyTable_final_crg_post_overrides = compute_standalone_and_overrides(
        qual_cg_df,
        MISC_INPUT_DIR,
        STATIC_INPUT_DIR,
        NEW_INPUT_DIR,
    ...
    # Combine all key tables and save
