...
def interest_expense_run(sector_name):
    ...
    # --- Data processing ---
    interest_data, agg_interest_data, summary_steps, MEVdata, interest_data_df, int_expense_issue = (
        load_and_clean_data(IE_config)
    )

    # --- Modelling ---
    modelling_results = run_modelling(
        interest_data, agg_interest_data, MEVdata, int_expense_issue, IE_config
    )

    # --- Backtesting ---
    backtest_results = run_backtesting(
        interest_data, agg_interest_data, MEVdata, modelling_results, int_expense_issue, IE_config
    )
